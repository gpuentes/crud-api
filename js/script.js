...
async function loadVehicles(search = "") {
    ...
    await $.ajax({
        ...
        success: data => {
                    ...
                    '<div class="card car-card">' +
                        ...
                        '<div class="card-body">' +
                            '<div class="row">' +
                                ...
                                '<div class="col-sm-3">' +
                                    '<button " ' +
                                        'onClick="seeDetails('+ vehicle.id +')"><img src="https://img.icons8.com/pastel-glyph/64/000000/edit.png"/></button>' +
                                '</div>' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
            ...
            if(data.length === 0) $("#vehicles-div").html("Não encontramos itens nesse base de dados")
            else $("#vehicles-div").html(vehicleDiv)
        },
        error: err => {
            $("#vehicles-div").html("<p>Houve um erro ao buscar os itens no banco</p>")
        }
    })
